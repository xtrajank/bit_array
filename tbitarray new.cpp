...
int main() {
   ...
   test_(b9.count() == 28);
   BitArray<> b9long {b9};	// We will use this a bit later; see below
   nothrow_(b9 <<= 2);
   test_(b9.count() == 26);
   nothrow_(b9 >>=33);   
   test_(b9.count() == 23);
   ...
   // Test longer arrays, and a big shift
   b9long += b9long;
   test_(b9long.size() == 112);
   test_(b9long.count() == 56);
   b9long += b9long;
   test_(b9long.size() == 224);
   b9long <<= 129;
   test_(b9long.count() == 39);
